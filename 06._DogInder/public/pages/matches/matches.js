...
function createMatchesProfile(dog) {
    ...
    setupHammerPanEvents(dogMatchesImageImg) // the hammer events are set on the profile pictures
}
...
function setupHammerPanEvents(dogImageTag) {
    ...
    hammertime.on('pan', (event) => { // left/right
    ...
    hammertime.on('panend', (event) => { // when lifting finger from touchbar 
        if (event.deltaX > 0) {
