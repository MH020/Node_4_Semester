 ...
 */
  function computeIntervalInputData (session, input) {
    ...
    if (deltaTime > COMPUTE_INTERVAL || last.velocity === undefined) {
      const deltaX = last.deltaX - input.deltaX
      const deltaY = last.deltaY - input.deltaY
 ...
 */
  ...
  const MOUSE_WINDOW_EVENTS = 'mousemove mouseout mouseup'
 ...
 */
  ...
  inherit(MouseInput, Input, {
    ...
    handler: function (ev) {
      ...
      // out of the window?
      const target = ev.relatedTarget || ev.toElement || ev.target
  ...
  let POINTER_WINDOW_EVENTS = 'pointermove pointerout pointerup pointercancel'
 ...
 */
  ...
  inherit(PointerEventInput, Input, {
    ...
    handler: function (ev) {
      ...
      const storeIndex = inArray(store, ev.pointerId, 'pointerId')
      if (storeIndex < 0) {
        return
      }
 ...
 */
  ...
  inherit(TouchInput, Input, {
     ...
     */
    handler: function (ev) {
      const touches = normalizeTouches(ev, this)
 ...
 */
  function normalizeTouches (ev, touchInput) {
    ...
    const targetIds = touchInput.targetIds
    const targetTouches = toArray(ev.targetTouches)
 ...
 */
  ...
  Recognizer.prototype = {
     ...
     */
    canEmit: function () {
      for (let i = 0; i < this.requireFail.length; i++) {
    ...
    recognize: function (inputData) {
      ...
      const inputDataClone = extend({}, inputData)
 ...
 */
  ...
  Manager.prototype = {
    ...
    recognize: function (inputData) {
      ...
      for (let i = 0, len = this.recognizers.length; i < len; i++) {
 ...
 */
  function toggleCssProps (manager, add) {
    ...
    const cssProps = manager.options.cssProps

    each(cssProps, function (value, name) {
      element.style[prefixed(element.style, name)] = add ? value : ''
    })

    const falseFn = add && function () { return false }
